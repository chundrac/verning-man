...
fit <- brm(formula = response ~ date + I(date^3) + (1+date+I(date^3)|dialect_ID) + (1+date+I(date^3)|dialect_lemma_ID) + (1+date+I(date^3)|dialect_infl_ID) + (1+date+I(date^3)|lemma_macroinfl_ID) + (1+date+I(date^3)|dialect_exp_out_ID), 
           data=verner.df, 
           family=bernoulli(), 
...
save.image('BRMS_fit_cub_no_quad.Rmd')