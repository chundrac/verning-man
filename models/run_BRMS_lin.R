...
fit <- brm(formula = response ~ date + (1+date|dialect_ID) + (1+date|dialect_lemma_ID) + (1+date|dialect_infl_ID) + (1+date|lemma_macroinfl_ID) + (1+date|dialect_exp_out_ID), 
           data=verner.df, 
           family=bernoulli(), 
