...
fit <- brm(formula = response ~ date + I(date^2) + (1+date+I(date^2)|dialect_ID) + (1+date+I(date^2)|dialect_lemma_ID) + (1+date+I(date^2)|dialect_infl_ID) + (1+date+I(date^2)|lemma_macroinfl_ID) + (1+date+I(date^2)|dialect_exp_out_ID), 
           data=verner.df, 
           family=bernoulli(), 
